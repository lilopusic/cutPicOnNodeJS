...
(function ($) {
    ...
    $.fn.cutPic = function (userOptions) {
        ...
        var uploadFunc = null;
        //下载的默认格式为png
        var downloadType = void 0;
        //下载的默认名字为cutPic
        var downloadName = void 0;
        //输入框，恒定一个
        var inputBox = void 0;
        //别名
        var _self = this;
        ...
        if (typeof uploadFunc !== 'function') {
            switch (uploadFunc) {
                ...
                default:
                    uploadFunc = function uploadFunc() {
                        layer.open({
                            content: '没有找到对应上传方法'
                        });
                    };
         ...
         */
        var createContainer = function createContainer(canvas) {
            //设置原图
            ...
            //背景
            var $backGround = $('<div class=' + backGroundClass + '></div>');
            $backGround.css("background-image", 'url(' + image.src + ')');
            $backGround.css("position", "relative");
            $backGround.height(height).width(width);
            //绘画层
            var $paintCanvas = $('<canvas id=' + paintCanvasId + '>your browser doesn\'t support canvas!</canvas>');
            $paintCanvas.css("position", "absolute").css("top", 0).css("left", 0).css("z-index", 0);
            $paintCanvas.attr("height", height).attr("width", width);
            //截图层
            var $pic = $('<canvas id=' + picId + '>your browser doesn\'t support canvas!</canvas>');
            $pic.css("position", "absolute").css("top", 0).css("left", 0).css("z-index", 10);
            $pic.attr("height", height).attr("width", width);
            //遮罩层
            var $mask = $('<div class=' + maskClass + '></div>');
            $mask.css("position", "relative").css("display", "none").css("top", 0).css("left", 0).css("background-color", "rgba(0,0,0,0.8)");
            $mask.height(height).width(width);
            $mask.append($pic).append($paintCanvas);
            //截图框层
            var $myCanvas = $('<canvas id=' + myCanvasId + '>your browser doesn\'t support canvas!</canvas>');
            $myCanvas.css("position", "relative").css("cursor", "crosshair").css("top", 0).css("left", 0);
        ...
        //获取base64格式图片
        //暴露给外部
        var getCutPicBase64 = function getCutPicBase64(type) {
        ...
        function resolveMosaic() {
            ...
            if (!cutPicCanvas.isMosaic) {
                ...
                if (step == 0 || step == 2) {
                    ctxPic.drawImage(image, x0, y0, cutPicWidth, cutPicHeight, -(cutPicCanvas.width / cutPicCanvas.zoom) / 2, -(cutPicCanvas.height / cutPicCanvas.zoom) / 2, cutPicCanvas.width / cutPicCanvas.zoom, cutPicCanvas.height / cutPicCanvas.zoom);
                }
                if (step == 1 || step == 3) {
                    ctxPic.drawImage(image, x0, y0, cutPicWidth, cutPicHeight, -(cutPicCanvas.height / cutPicCanvas.zoom) / 2, -(cutPicCanvas.width / cutPicCanvas.zoom) / 2, cutPicCanvas.height / cutPicCanvas.zoom, cutPicCanvas.width / cutPicCanvas.zoom);
                }
                ctxPic.restore();
                if (cutPicCanvas.isReverse) {
                    reverse();
        ...
        function mosaic() {
            ...
            for (var r = 0; r < numTileRows; r++) {
                for (var _c = 0; _c < numTileCols; _c++) {
                    var tx = _c * tileWidth + tileWidth / 2;
        ...
        function bindMouseEvent() {
            ...
            paintC.onmousedown = function (event) {
                ...
                var x = position.x,
                    y = position.y;
                if (x > cutPicCanvas.left && x < cutPicCanvas.left + cutPicCanvas.width && y > cutPicCanvas.top && y < cutPicCanvas.top + cutPicCanvas.height) {
        ...
        function bindWheelEvent() {
            ...
            c2.addEventListener("wheel", function (event) {
                ...
                var position = getEventPosition(event);
                //在截取区域中才响应
                if (position.x > cutPicCanvas.left && position.x < cutPicCanvas.left + cutPicCanvas.width && position.y > cutPicCanvas.top && position.y < cutPicCanvas.top + cutPicCanvas.height) {
                    ...
                    if (event.wheelDelta == -120 || event.wheelDelta == -3) {
                        ...
                        var _left = (c2.width - cutPicCanvas.width) / 2;
                        var _top = (c2.height - cutPicCanvas.height) / 2;
                        cutPicCanvas.left = _left;
                        cutPicCanvas.top = _top;
                        ctxPic.scale(cutPicCanvas.zoom, cutPicCanvas.zoom);
